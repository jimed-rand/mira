...
|---|---|---|---|---|---|---|
| 1 | `Reserved` | — | 1 MiB | 64 MiB | raw / bootloader | — |
| 2 | `ESP` | `BOOT` | 65 MiB | 256 MiB | FAT32 | `esp` |
| 3 | `ROOT` | `ROOT` | 321 MiB | Variable | ext4 | — |

> The first 64 MiB (`Reserved`) are protected to accommodate the vendor bootloader area expected by the Amlogic ROM without conflicting with the primary GPT tables.

### Deployment Layout (mira-install)

The installer always places the boot stack (Reserved + ESP + SWAP) on the **eMMC**, regardless of where the root filesystem lives. This ensures the STB can always find and execute the GRUB bootloader.

#### Target: eMMC

The entire eMMC is re-partitioned as follows:

| Partition | Name | Label | Size | Filesystem | Notes |
|---|---|---|---|---|---|
| `p1` | `Reserved` | — | 64 MiB | raw | Protects bootloader binary area |
| `p2` | `ESP` | `BOOT_TARGET` | 128 MiB | FAT32 | UEFI / GRUB entrypoint |
| `p3` | `SWAP` | `SWAP_TARGET` | 3072 MiB | linux-swap | Fixed swap partition |
| `p4` | `ROOT` | `ROOT_TARGET` | 100% remaining | ext4 / xfs | Main root filesystem |

#### Target: SD Card

The **eMMC** is re-partitioned to hold boot and swap, and the **SD Card** holds only the root filesystem:
...
| `p1` | `Reserved` | — | 64 MiB | raw | Protects bootloader binary area |
| `p2` | `ESP` | `BOOT_TARGET` | 128 MiB | FAT32 | UEFI / GRUB entrypoint |
| `p3` | `SWAP` | `SWAP_TARGET` | 100% remaining | linux-swap | Entire remaining eMMC used as swap |

**SD Card** (root only):

| Partition | Name | Label | Size | Filesystem | Notes |
|---|---|---|---|---|---|
| `p1` | `ROOT` | `ROOT_TARGET` | 100% | ext4 / xfs | Entire SD Card used as root filesystem |
